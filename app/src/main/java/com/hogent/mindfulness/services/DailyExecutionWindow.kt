 ...
 */
class DailyExecutionWindow(currentHour: Int, currentMinute: Int, targetHour: Long, targetMinute: Long, windowLengthInMinutes: Long) {
    val startMs: Long
    val endMs: Long
    ...
    init {
        ...
        val hourOffset: Long
        val minuteOffset: Long

        if (targetHour == currentHour.toLong() && targetMinute < currentMinute) {
            hourOffset = TimeUnit.HOURS.toMillis(23)
        } else if (targetHour - currentHour == 1L) { // if we are less then an hour ahead, but into the next hour
            // move forward to 0 minute of next hour
            hourOffset = TimeUnit.MINUTES.toMillis(60 - currentMinute.toLong())
            currentMinute = 0
